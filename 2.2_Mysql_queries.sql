...
/* 21 */ SELECT p.nombre, p.precio, f.nombre FROM producto AS p, fabricante AS f WHERE p.codigo_fabricante = f.codigo;
/* 22 */ SELECT p.nombre, p.precio, f.nombre FROM producto AS p, fabricante AS f WHERE p.codigo_fabricante = f.codigo ORDER BY f.nombre;
/* 23 */ SELECT p.codigo, p.nombre, f.codigo, f.nombre FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante;
/* 24 */ SELECT p.nombre, p.precio, f.nombre FROM producto AS p, fabricante AS f WHERE p.codigo_fabricante = f.codigo  ORDER BY p.precio LIMIT 1;
/* 25 */ SELECT p.nombre, p.precio, f.nombre FROM producto AS p, fabricante AS f WHERE p.codigo_fabricante = f.codigo  ORDER BY p.precio DESC LIMIT 1;
/* 26 */ SELECT p.* FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND f.nombre = 'Lenovo';
/* 27 */ SELECT p.* FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND f.nombre = 'Crucial' AND p.precio > 200;
/* 28 */ SELECT p.* FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND (f.nombre = 'Asus' OR f.nombre = 'Hewlett-Packard' OR f.nombre = 'Seagate');
/* 29 */ SELECT p.* FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND f.nombre IN ('Asus', 'Hewlett-Packard', 'Seagate');
/* 30 */ SELECT p.nombre, p.precio FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND f.nombre LIKE '%e';
/* 31 */ SELECT p.nombre, p.precio, f.nombre FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND f.nombre LIKE '%w%';
/* 32 */ SELECT p.nombre, p.precio, f.nombre FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND p.precio >= 180 ORDER BY p.precio DESC, p.nombre ASC;
/* 33 */ SELECT DISTINCT f.codigo, f.nombre FROM fabricante AS f JOIN producto AS p ON f.codigo = p.codigo_fabricante;
/* 34 */ SELECT f.*, p.* FROM fabricante AS f LEFT JOIN producto AS p ON f.codigo = p.codigo_fabricante;
/* 35 */ SELECT DISTINCT f.* FROM fabricante AS f WHERE (SELECT COUNT(*) FROM producto WHERE codigo_fabricante = f.codigo) = 0;
...
/* 38 */ SELECT p.nombre FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND precio = (SELECT MAX(precio) FROM producto AS p WHERE f.codigo = codigo_fabricante AND f.nombre = 'Lenovo');
/* 39 */ SELECT p.nombre FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND precio = (SELECT MIN(precio) FROM producto AS p WHERE f.codigo = codigo_fabricante AND f.nombre = 'Hewlett-Packard');
/* 40 */ SELECT * FROM producto WHERE precio >= (SELECT MAX(p.precio) FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND f.nombre = 'Lenovo');
/* 41 */ SELECT p.* FROM producto AS p, fabricante AS f WHERE f.codigo = p.codigo_fabricante AND f.nombre = 'Asus' AND p.precio > (SELECT AVG(precio) FROM producto WHERE codigo_fabricante = f.codigo);
